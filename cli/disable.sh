...
function usage() {
    cat << EOF
Usage: profile-manager disable [OPTIONS]

Options:
  -n, --name                Name of the profile to be disabled.
  -h, --help                Show this help message.

Examples:
  profile-manager disable --name NAME
  profile-manager disable -n NAME
...
if [ ! -d "${profile_storage_directory}" ] ; then
    echo "There is no profile installed with the name '${name}'."
    exit 1
fi

if [ ! -h "${profile_active_link}" ] ; then
    echo "The profile '${name}' is already disabled."
    exit 1
fi
